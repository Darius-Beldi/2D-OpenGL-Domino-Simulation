    ...
    // Adaug? defini?ia pentru TexCoords înainte de utilizare în CreateVBO
    GLfloat TexCoords[] = {
    ...
    void RenderFunction(void)
    {
        ...
        codCol = 0; // Use table texture
        glUniform1i(codColLocation, codCol);

        // Table quad: from (0,0) to (a,GROUND_Y)
        glm::mat4 tableMatrix = glm::mat4(1.0f);
        tableMatrix = glm::translate(tableMatrix, glm::vec3(0.0f, 0.0f, 0.0f));
        tableMatrix = glm::scale(tableMatrix, glm::vec3(a, GROUND_Y, 1.0f));
        glm::mat4 finalTableMatrix = resizeMatrix * tableMatrix;
        glUniformMatrix4fv(myMatrixLocation, 1, GL_FALSE, &finalTableMatrix[0][0]);
        glDrawArrays(GL_QUADS, 12, 4);

        // --- Draw dominoes with texture ---
        ...
        for (int i = 0; i < NUM_DOMINOS; i++) {
            ...
            codCol = 1; // Use domino texture
            glUniform1i(codColLocation, codCol);
            glDrawArrays(GL_QUADS, 0, 4);

            // Contur domino (no texture)
            codCol = 0;
            glUniform1i(codColLocation, codCol);
            glLineWidth(2.5f);
